...
async def test_delete_user_by_privilege_roles(
    ...
    assert resp.status_code == 200
    data_from_resp = resp.json()
    assert data_from_resp == {"deleted_user_id": str(user_data_for_deletion["user_id"])}
...
async def test_reject_delete_superadmin(
    ...
    data_from_resp = resp.json()
    assert data_from_resp == {"detail": "Superadmin cannot be deleted via API"}
    users_from_db = await get_user_from_database(user_data["user_id"])
    user_from_db = dict(users_from_db[0])
    assert user_from_db["name"] == user_data["name"]
    assert user_from_db["surname"] == user_data["surname"]
    ...
    assert user_from_db["user_id"] == user_data["user_id"]
    assert PortalRole.ROLE_PORTAL_SUPERADMIN in user_from_db["roles"]
